...
{
#开始处理版本号
	cktversion=`getprop ckt.internal.version`
...
handleforvegeta01a()
{
      if [ "$custname" = "fnac" ]
      then
      	      cktversion_softcode="S14A"
             cktversion_customName="FNAC"
      elif [ "$custname" = "planeta" ]
      then
      	      cktversion_softcode="S18A"
             cktversion_customName="PLANETA"
      fi
}
...
SetAllProp()
{
	...
	setprop ckt.internal.version ${cktversion_project}-${cktversion_softcode}_${cktversion_customName}_${cktversion_language}_${cktversion_ver}_${cktversion_longtime}
	setprop ro.build.display.id 2.0.0_20160425-0944 
	...
	setprop ro.bq.version.release ${cktversion_project}-${cktversion_softcode}_${cktversion_customName}_${cktversion_language}_${cktversion_ver}_${cktversion_shorttime}
#需要调试的时候打开
	printf "ro.ckt.type=${type}\n" >>$logfile
	printf "ckt.internal.version ${cktversion_project}-${cktversion_softcode}_${cktversion_customName}_${cktversion_language}_${cktversion_ver}_${cktversion_longtime}\n" >>$logfile
	printf "ro.build.display.id 2.0.0_20160425-0944"
	...
	printf "ro.bq.version.release ${cktversion_project}-${cktversion_softcode}_${cktversion_customName}_${cktversion_language}_${cktversion_ver}_${cktversion_shorttime}\n" >>$logfile
}
...
handlectpforektf()
{
	...
	then
		...
		elan_iap $filename 0x10 3k >>$logfile
	else
...
handlectpforft()
{
	...
	then
		...
		ft5336_upgrade $filename >>$logfile
	else
