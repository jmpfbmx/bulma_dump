...
#开始处理版本号
	...
	cktversion_shorttime=${cktversion_longtime:0:${#cktversion_longtime}-6}
       custname=`getprop ro.product.cust`
...
handlectpforft()
{
	...
	bigestver=0;
	filename="";
	project_folder=$cktversion_project;
	if [ "$project_folder" = "VEGETA01A" ]
	then
		project_folder="VEGETAFHD"
	fi
	for i in system/vendor/$ctpic/$project_folder/$ctpvendor/*.*
	do
		if [ ! -e $i ]
		then
			printf "$i no exist\n">>$logfile;
...
#
if [ "$cktversion_project" = "VEGETA01A" ]
then
	UpdateCTP
fi
